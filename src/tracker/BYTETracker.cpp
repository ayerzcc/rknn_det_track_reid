...
{
    tracked_stracks_.erase(tracked_stracks_.begin(), tracked_stracks_.end());
    lost_stracks_.erase(lost_stracks_.begin(), lost_stracks_.end());
    removed_stracks_.erase(removed_stracks_.begin(), removed_stracks_.end());
}

const std::vector<byte_track::BYTETracker::STrackPtr>& byte_track::BYTETracker::getTrackedStracks() const
{
    return tracked_stracks_;
}

const std::vector<byte_track::BYTETracker::STrackPtr>& byte_track::BYTETracker::getLostStracks() const
{
    return lost_stracks_;
}

size_t byte_track::BYTETracker::getFrameId() const
{
    return frame_id_;
}

size_t byte_track::BYTETracker::getMaxTimeLost() const
{
    return max_time_lost_;
}

std::string byte_track::BYTETracker::getLastDebugJson() const
{
    return last_debug_json_;
}

bool byte_track::BYTETracker::recoverTrack(const STrackPtr& track, const Object& object)
{
    if (!track)
    {
        return false;
...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
    ...
    {
        const auto strack = std::make_shared<STrack>(object.rect, object.prob, object.feature);
    ...
    std::vector<std::vector<int>> matches1_idx;
    std::vector<int> unmatch_detection1_idx, unmatch_track1_idx;
    std::vector<std::vector<int>> matches2_idx;
    std::vector<int> unmatch_track2_idx, unmatch_detection2_idx;
    std::vector<std::vector<int>> matches3_idx;
    std::vector<int> unmatch_unconfirmed_idx;
    std::vector<int> unmatch_detection3_idx;
    std::vector<std::vector<float>> iou_dist1;

    {
        ...
        linearAssignment(dists, strack_pool.size(), det_stracks.size(), match_thresh_,
                         matches1_idx, unmatch_track1_idx, unmatch_detection1_idx);

        for (const auto &match_idx : matches1_idx)
        {
        ...
        for (const auto &unmatch_idx : unmatch_detection1_idx)
        {
            remain_det_stracks.push_back(det_stracks[unmatch_idx]);
        }

        for (const auto &unmatch_idx : unmatch_track1_idx)
        {
...

    {
        ...
        linearAssignment(dists, remain_tracked_stracks.size(), det_low_stracks.size(), 0.5,
                         matches2_idx, unmatch_track2_idx, unmatch_detection2_idx);

        for (const auto &match_idx : matches2_idx)
        {
        ...
        for (const auto &unmatch_track : unmatch_track2_idx)
        {
...

    {
        ...
        linearAssignment(dists, non_active_stracks.size(), remain_det_stracks.size(), 0.7,
                         matches3_idx, unmatch_unconfirmed_idx, unmatch_detection3_idx);

        for (const auto &match_idx : matches3_idx)
        {
        ...
        // Add new stracks
        for (const auto &unmatch_idx : unmatch_detection3_idx)
        {
    ...
    dbg << "{\"frame\":" << frame_id_ << ",\"pool_ids\":[";
    for (size_t i = 0; i < strack_pool.size(); ++i) { dbg << strack_pool[i]->getTrackId(); if (i + 1 < strack_pool.size()) dbg << ","; }
    dbg << "],\"high_det_scores\":[";
    for (size_t i = 0; i < det_stracks.size(); ++i) { dbg << det_stracks[i]->getScore(); if (i + 1 < det_stracks.size()) dbg << ","; }
    dbg << "],\"matches1\":[";
    for (size_t i = 0; i < matches1_idx.size(); ++i) { dbg << "[" << matches1_idx[i][0] << "," << matches1_idx[i][1] << "]"; if (i + 1 < matches1_idx.size()) dbg << ","; }
    dbg << "],\"u_track1\":[";
    for (size_t i = 0; i < unmatch_track1_idx.size(); ++i) { dbg << unmatch_track1_idx[i]; if (i + 1 < unmatch_track1_idx.size()) dbg << ","; }
    dbg << "],\"u_det1\":[";
    for (size_t i = 0; i < unmatch_detection1_idx.size(); ++i) { dbg << unmatch_detection1_idx[i]; if (i + 1 < unmatch_detection1_idx.size()) dbg << ","; }
    dbg << "],\"iou_dist1\":[";
    for (size_t i = 0; i < iou_dist1.size(); ++i) {
        dbg << "[";
        for (size_t j = 0; j < iou_dist1[i].size(); ++j) {
            dbg << iou_dist1[i][j];
            if (j + 1 < iou_dist1[i].size()) dbg << ",";
        }
        dbg << "]";
        if (i + 1 < iou_dist1.size()) dbg << ",";
    }
    dbg << "],\"iou_dist1_track4_det2\":";
    constexpr int kFocusTrackId = 4;
    constexpr size_t kFocusDetIndex = 2;
    bool focus_pair_found = false;
    for (size_t i = 0; i < strack_pool.size(); ++i)
    {
        if (strack_pool[i]->getTrackId() != kFocusTrackId)
        {
            continue;
        }
        if (i < iou_dist1.size() && kFocusDetIndex < iou_dist1[i].size())
        {
            dbg << iou_dist1[i][kFocusDetIndex];
            focus_pair_found = true;
        }
        break;
    }
    if (!focus_pair_found) dbg << "null";
    dbg << ",\"matches2\":[";
    for (size_t i = 0; i < matches2_idx.size(); ++i) { dbg << "[" << matches2_idx[i][0] << "," << matches2_idx[i][1] << "]"; if (i + 1 < matches2_idx.size()) dbg << ","; }
    dbg << "],\"matches3\":[";
    for (size_t i = 0; i < matches3_idx.size(); ++i) { dbg << "[" << matches3_idx[i][0] << "," << matches3_idx[i][1] << "]"; if (i + 1 < matches3_idx.size()) dbg << ","; }
    dbg << "],\"refind_ids\":[";
    for (size_t i = 0; i < refind_stracks.size(); ++i) { dbg << refind_stracks[i]->getTrackId(); if (i + 1 < refind_stracks.size()) dbg << ","; }
    dbg << "],\"tracked_ids\":[";
    for (size_t i = 0; i < tracked_stracks_.size(); ++i) { dbg << tracked_stracks_[i]->getTrackId(); if (i + 1 < tracked_stracks_.size()) dbg << ","; }
    dbg << "],\"lost_ids\":[";
    for (size_t i = 0; i < lost_stracks_.size(); ++i) { dbg << lost_stracks_[i]->getTrackId(); if (i + 1 < lost_stracks_.size()) dbg << ","; }
    dbg << "]}";
